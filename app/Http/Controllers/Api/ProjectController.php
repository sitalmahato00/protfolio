...
class ProjectController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // Keywords are auto-generated via model boot (HasSeoAttributes)
    ...
    public function update(Request $request, Project $project)
    {
        ...
        // If title changed and no slug given, regenerate slug
        if (!empty($data['title']) && $data['title'] !== $project->title && empty($data['slug'])) {
    ...
    public function deleteImage(Request $request, Project $project)
    {
        ...
        $paths = collect($project->images ?? [])
            ->filter(fn($p) => $p !== $request->image)
            ->values()
            ->toArray();
