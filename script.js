...
(function initMusic() {
  ...
  // Create ambient synth with Web Audio API (no external file needed)
  let audioCtx, oscillators = [], playing = false;

  function createAmbient() {
    audioCtx = new (window.AudioContext || window.webkitAudioContext)();

    const freqs = [55, 110, 165, 220]; // A chord — ambient
    const masterGain = audioCtx.createGain();
    masterGain.gain.setValueAtTime(0, audioCtx.currentTime);
    masterGain.gain.linearRampToValueAtTime(0.06, audioCtx.currentTime + 2);
    masterGain.connect(audioCtx.destination);

    // Reverb-like using convolver substitute
    freqs.forEach((freq, i) => {
      const osc  = audioCtx.createOscillator();
      const gain = audioCtx.createGain();

      osc.type = i % 2 === 0 ? 'sine' : 'triangle';
      osc.frequency.setValueAtTime(freq + Math.random() * 2, audioCtx.currentTime);
      ...
      gain.gain.setValueAtTime(0.4 - i * 0.07, audioCtx.currentTime);
      osc.connect(gain);
