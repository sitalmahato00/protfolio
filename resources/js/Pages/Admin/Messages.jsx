    ...
    return (
        <div onClick={onClick}
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                {/* Avatar */}
                <div style={{ width: '36px', height: '36px', borderRadius: '50%', background: 'linear-gradient(135deg,#6366F1,#8B5CF6)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: '13px', fontWeight: '700', flexShrink: 0 }}>
...
export default function AdminMessages() {
    ...
    function selectMsg(msg) { setActive(msg); if (!msg.is_read) markRead(msg.id); }
    ...
    return (
        <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '320px 1fr', gap: '14px', height: 'calc(100vh - 140px)' }}>

                {/* ── Left: inbox list ── */}
                <div style={{ background: cardBg, border: cardBorder, borderRadius: '14px', boxShadow: cardShadow, padding: '16px', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                    {/* Header */}
                ...
                <div style={{ background: cardBg, border: cardBorder, borderRadius: '14px', boxShadow: cardShadow, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                    {!active ? (
                        /* Empty state */
                        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: t.textMuted, padding: '40px' }}>
                            <div style={{ width: '64px', height: '64px', borderRadius: '16px', background: dark ? 'rgba(99,102,241,0.12)' : 'rgba(79,70,229,0.07)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '28px', marginBottom: '16px' }}>
                                💌
                            </div>
                        ...
                        <>
                            {/* Detail header */}
                            <div style={{ padding: '18px 22px', borderBottom: `1px solid ${divider}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px', flexWrap: 'wrap' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                    <div style={{ width: '42px', height: '42px', borderRadius: '50%', background: 'linear-gradient(135deg,#6366F1,#8B5CF6)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: '16px', fontWeight: '700', flexShrink: 0 }}>
                                        {active.name?.[0]?.toUpperCase()}
                                    </div>
                                    <div>
                                        <div style={{ fontSize: '15px', fontWeight: '700', color: dark ? '#F1F5F9' : '#111827' }}>{active.name}</div>
                                        <div style={{ fontSize: '12px', color: t.textMuted }}>{active.email}</div>
                                    </div>
                                </div>
                                <div style={{ display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap' }}>
                                    <span style={{ fontSize: '11px', color: t.textMuted }}>{new Date(active.created_at).toLocaleString()}</span>
                                    ...
                                    <button onClick={() => remove(active.id)} style={{ padding: '7px 14px', fontSize: '12px', borderRadius: '9px', background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.2)', color: '#EF4444', cursor: 'pointer', fontWeight: '600' }}>Delete</button>
                                </div>
                            </div>
                            ...
                            <div style={{ flex: 1, padding: '24px 28px', overflowY: 'auto' }}>
                                {active.subject && (
                                    <div style={{ marginBottom: '18px' }}>
                                        ...
                                        <div style={{ fontSize: '17px', fontWeight: '700', color: dark ? '#F1F5F9' : '#111827' }}>{active.subject}</div>
                                    </div>
                                )}

                                {/* Message bubble */}
                                <div style={{ background: dark ? 'rgba(255,255,255,0.04)' : '#F9FAFB', border: `1px solid ${divider}`, borderRadius: '12px', padding: '20px 22px', lineHeight: 1.7, fontSize: '14px', color: dark ? '#CBD5E1' : '#374151' }}>
                                    {active.message}
                                </div>
